...
import dev.enricosola.porcellino.exception.user.InvalidPendingEmailUserException;
...
public class EmailVerificationTokenService {
    ...
    public String generate(User user) {
        if ( user.getPendingEmail() == null || user.getPendingEmail().isBlank() ) {
            throw new InvalidPendingEmailUserException("User does not have a pending email address.");
        }
        Date expire = new Date(System.currentTimeMillis() + (this.tokenTTL * 1000L));
        return this.verificationTokenService.generate(user.getPendingEmail(), EmailVerificationTokenService.VERIFICATION_TOKEN_SCOPE, expire);
    }
     ...
     * @throws InvalidPendingEmailUserException If the user does not have a pending email address.
     */
    public boolean validate(User user, String token) {
        if ( user.getPendingEmail() == null || user.getPendingEmail().isBlank() ) {
            throw new InvalidPendingEmailUserException("User does not have a pending email address.");
        }
        return this.verificationTokenService.validate(user.getPendingEmail(), EmailVerificationTokenService.VERIFICATION_TOKEN_SCOPE, token);
    }
